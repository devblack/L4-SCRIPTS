...
int main(int argc, char * argv[])
{
  ...
  char ip_d[20];
    sprintf(ip_d, "%d.%d.%d.%d", iprang[rand() % 5], rand() % 252, rand() % 254, rand() % 250);
    char * SOURCE = ip_d;
    int s_port = 40669 + (rand() % 13665);
    saddr.sin_port = htons(s_port);
    inet_pton(AF_INET, SOURCE, (struct in_addr *)&saddr.sin_addr.s_addr);
    ip->id = 0;    /* default value */
