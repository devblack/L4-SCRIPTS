...
/*    udp->check = udp_checksum(
            ...
            udp,
           (const uint16_t *)udp);
osendto(s, (char *)packet, sizepacket, 0,(struct sockaddr *)&daddr, (socklen_t)sizeof(daddr)) ;   
#################
...
//      usleep(10000);
        ...
        udp->len = htons(sizeof(struct udphdr) + 4);
        memset(((void *) udp) + sizeof(struct udphdr), 'A', 4);
	packet_bots4[0] = (char)rand()%200;
        packet_bots4[1] = 0x1e;
        packet_bots4[2] = (char)rand()%211;;
        packet_bots4[3] = (char)rand()%154;
  memcpy((void *)udp + sizeof(struct udphdr), (char *)packet_bots4, 4);
    udp->check = 0;
    udp->check = udp_checksum(
            ip,
            udp,
      ...
      sendto(s, (char *)packet, sizepacket, 0,(struct sockaddr *)&daddr, (socklen_t)sizeof(daddr)) ;
        ...
        sizepacket = sizeof(struct iphdr) + sizeof(struct udphdr) + 6;
        ip->tot_len = htons(sizeof(sizepacket));    /* 16 byte value */
  ...
  memcpy((void *)udp + sizeof(struct udphdr), (char *)packet_bots6, 6);
    udp->check = 0;
    udp->check = udp_checksum(
            ip,
            udp,
