...
int main(int argc, char *argv[ ])
{
    ...
    if(argc < 6){
        ...
        fprintf(stdout, "\x1b[0;36mYo nigga run this:\x1b[1;35m %s 1.0.0.0 255.255.255.255 ntp.txt 2 1ms \n", argv[0]);
        exit(-1);
    }
    fd = fopen(argv[3], "a");
    ...
    sleep(1);
    printf("\x1b[0;36mMade By J4CK.. \x1b[1;35m Scanning..\n");
    char *temp = (char *)malloc(17);
    memset(temp, 0, 17);
    sprintf(temp, "\x1b[0;36mNTP Found");
    printf("%-16s", temp);
    memset(temp, 0, 17);
    sprintf(temp, "\x1b[1;34m  IP/s");
    printf("%-16s", temp);
    memset(temp, 0, 17);
    sprintf(temp, "\x1b[0;34mmBytes/s");
    printf("%-16s", temp);
    memset(temp, 0, 17);
    sprintf(temp, "\x1b[1;33mThreads");
    printf("%-16s", temp);
    memset(temp, 0, 17);
    sprintf(temp, "\x1b[0;31mPercent Done");
    printf("%s", temp);
    printf("\x1b[0;36m Follow Me On insta!\x1b[1;35m @flexingonlamers\n");
