...
int main(int argc, char *argv[ ])
{
    ...
    if(argc < 6){
      fprintf(stderr, "\x1b[1;35mLooking for eskimo babies to sell on the darkweb.. \x1b[1;33mscanner made by \x1b[1;34m@flexingonlamers\n");
      fprintf(stdout, "\x1b[1;34mEnter this to scan big lists my dood:\x1b[0;36m %s 1.0.0.0 255.255.255.255 portmap.txt 2 1ms\n", argv[0]);
      exit(-1);
    }
    fd = fopen(argv[3], "a");
    ...
    sleep(1);
    printf("\x1b[1;33mLooking for eskimobabies to sell on the darkweb.. \n");
    char *temp = (char *)malloc(17);
    memset(temp, 0, 17);
    sprintf(temp, "\x1b[1;35mEskimoBabies Found    ");
    printf("%-16s", temp);
    memset(temp, 0, 17);
    sprintf(temp, "\x1b[0;36mIP/s");
    printf("%-16s", temp);
    memset(temp, 0, 17);
    sprintf(temp, "\x1b[1;35mBytes/s");
    printf("%-16s", temp);
    memset(temp, 0, 17);
    sprintf(temp, "\x1b[0;36mThreads");
    printf("%-16s", temp);
    memset(temp, 0, 17);
    sprintf(temp, "\x1b[1;35mPercent Done");
    printf("%s", temp);
    printf("\n");
